...
struct lista *
insertar (struct lista *lista1, char *listaelementos[], int index, int ntok)
{
  struct lista *nuevonodo;
  if ((nuevonodo = malloc (sizeof (struct lista))) == NULL)
    {
      borrarlista (lista1);
      liberarmemoria (listaelementos, ntok);
      warnx ("Error in malloc");
      exit (EXIT_FAILURE);
    }
  if ((nuevonodo->dir =
       malloc ((strlen (listaelementos[index]) + 1) * sizeof (char))) == NULL)
    {
      borrarlista (lista1);
      liberarmemoria (listaelementos, ntok);
      warnx ("Error in malloc");
  ...
  snprintf (nuevonodo->dir,
	    (strlen (listaelementos[index]) * sizeof (char) + 1), "%s",
	    listaelementos[index]);
...
main (int argc, char *argv[])
{
  ...
  for (int i = ntokens - 1; i > -1; i--)
    {
      listadirectorios = insertar (listadirectorios, pathtokens, i, ntokens);
    }
  liberarmemoria (pathtokens, ntokens);
  if (argc == 1)
    {
